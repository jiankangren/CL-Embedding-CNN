...
import datetime
import errno
from phishDetect.opensource.utils.metric import print_f_score, cal_FPR_TPR_all
...
from phishDetect.opensource.utils.mydatasets import H5Dataset
from .model import model_CharCNN_edit as CharCNN
...
parser.add_argument('--train_path', metavar='DIR',
                    help='path to training data csv', default='')
parser.add_argument('--val_path', metavar='DIR',
                    help='path to validation data csv', default='')
# learning
learn = parser.add_argument_group('Learning options')
learn.add_argument('--lr', type=float, default=0.0001, help='initial learning rate [default: 0.0001]')
learn.add_argument('--epochs', type=int, default=80, help='number of epochs for train [default: 200]')
...
experiment.add_argument('--val_interval', type=int, default=1500,
...
def train(train_loader, dev_loader, model, args):
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # validation
        val_loss, val_acc, TPR, FPR = eval(dev_loader, model, epoch, i_batch, optimizer, args)
...
def eval(data_loader, model, epoch_train, batch_train, optimizer, args):
    ...
    FPR, TPR = cal_FPR_TPR_all(predicates_all, target_all)

    print('\nEvaluation - loss: {:.6f}  lr: {:.5f}  acc: {:.4f}% ({}/{})  TPR: {:.4f}% FPR: {:.4f}% '.format(avg_loss,
                                                                                  optimizer.state_dict()[
                                                                                      'param_groups'][0]['lr'],
                                                                                  accuracy,
                                                                                  corrects,
                                                                                  size, TPR*100, FPR*100))
    print_f_score(predicates_all, target_all)
    print('\n')
    if args.log_result:

        with open(os.path.join(args.save_folder, 'result.csv'), 'a') as r:
            r.write('\n{:d},{:d},{:.5f},{:.5f},{:.5f},{:.5f},{:f}'.format(epoch_train,
                                                            batch_train,
                                                            avg_loss,
                                                            accuracy,
                                                            TPR,FPR,
                                                            optimizer.state_dict()['param_groups'][0]['lr']))
    return avg_loss, accuracy, TPR, FPR
...
if __name__ == '__main__':
    ...
    train_path = "/run/media/w/80EAEC20EAEC13E6/vector/train_csic_100_50_mo_req_remove.hdf5"
    test_path = "/run/media/w/80EAEC20EAEC13E6/vector/test_csic_100_50_mo_req_remove.hdf5"
    ...
    dev_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=0, drop_last=True)
    try:
    ...
    if args.log_result:
        with open(os.path.join(args.save_folder, 'result.csv'), 'w') as r:
            r.write('{:s},{:s},{:s},{:s},{:s}'.format('epoch', 'batch', 'loss', 'acc', 'tpr', 'fpr', 'lr'))
